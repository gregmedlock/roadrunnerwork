...
class TModel : IModel
{
	...
	public void initializeInitialConditions ()
	{
		...
		_init_y[1] = (double)0.012/ _c[0];
		_init_y[2] = (double)0.01/ _c[0];
		_init_y[3] = (double)0.01/ _c[0];
...
;
	...
	public void evalEvents (double timeIn, double[] oAmounts)
	{
		...
		if (supportFunctions._gt(_y[3],_y[1])
	 == 1.0) {
		...
		if (supportFunctions._gt(_y[0],(double)1/
	(double)40)
	 == 1.0) {
	...
	private void InitializeDelays() { 
		_eventDelay[0] = new TEventDelayDelegate(delegate { return (double)0.5; } );
		_eventType[0] = true;
