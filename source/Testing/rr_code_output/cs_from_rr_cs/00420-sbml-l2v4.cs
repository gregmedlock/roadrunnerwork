...
;
	...
	public void evalEvents (double timeIn, double[] oAmounts)
	{
	 ...
	 == 1.0) {
		...
		if (supportFunctions._gt(_y[2],(double)1.4)
	 == 1.0) {
	...
	private void InitializeDelays() { 
		_eventDelay[0] = new TEventDelayDelegate(delegate { return (double)5; } );
		_eventType[0] = true;
		_eventPersistentType[0] = true;
		_eventDelay[1] = new TEventDelayDelegate(delegate { return (double)10; } );
		_eventType[1] = true;
