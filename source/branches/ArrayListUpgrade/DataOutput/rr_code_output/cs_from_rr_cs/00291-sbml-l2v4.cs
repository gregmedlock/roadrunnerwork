...
class TModel : IModel
{
	...
	private double[] _gp = new double[4];           // Vector containing all the global parameters in the System  
	private double[] _sr = new double[0];           // Vector containing all the modifiable species references  
	private double[][] _lp = new double[3][];       // Vector containing all the local parameters in the System  
	private double[] _y = new double[5];            // Vector containing the concentrations of all floating species 
	private double[] _init_y = new double[5];            // Vector containing the initial concentrations of all floating species 
	private double[] _amounts = new double[5];      // Vector containing the amounts of all floating species 
	private double[] _bc = new double[0];           // Vector containing all the boundary species concentration values   
	private double[] _c = new double[2];            // Vector containing all the compartment values   
	private double[] _dydt = new double[5];         // Vector containing rates of changes of all species   
	private double[] _rates = new double[3];        // Vector containing the rate laws of all reactions    
	...
	string[] variableTable = new string[5];
	string[] boundaryTable = new string[0];
	string[] globalParameterTable = new string[4];
	int[] localParameterDimensions = new int[3];
	...
	public void convertToAmounts() {
		_amounts[0] = _y[0]*_c[1];
		_amounts[1] = _y[1]*_c[1];
		_amounts[2] = _y[2]*_c[1];
		_amounts[3] = _y[3]*_c[1];
		_amounts[4] = _y[4]*_c[1];
	}

	public void convertToConcentrations() {
		_y[0] = _amounts[0]/_c[1];
		_y[1] = _amounts[1]/_c[1];
		_y[2] = _amounts[2]/_c[1];
		_y[3] = _amounts[3]/_c[1];
		_y[4] = _amounts[4]/_c[1];
	}
	...
	{
		_init_y[0] = (double)1/ _c[1];
		_init_y[1] = (double)0/ _c[1];
		_init_y[2] = (double)1/ _c[1];
		_init_y[3] = (double)0.1/ _c[1];
		_init_y[4] = (double)0.5/ _c[1];
...
;
	...
	{
		_y[0] = oAmounts[0]/_c[1];
		_y[1] = oAmounts[1]/_c[1];
		_y[2] = oAmounts[2]/_c[1];
		_y[3] = oAmounts[3]/_c[1];
		_y[4] = oAmounts[4]/_c[1];
		...
		_dydt[0] = + 2*_rates[0] - _rates[0] - _rates[1];
		_dydt[1] = + _rates[1] - _rates[2];
		_dydt[2] = - _rates[0];
		convertToAmounts ();
	}
