...
class TModel : IModel
{
	...
	public void initializeInitialConditions ()
	{
		...
		_init_y[1] = (double)0.0012/ _c[0];
		_init_y[2] = (double)0.002/ _c[0];
		_init_y[3] = (double)0.001/ _c[0];
...
;
	...
	public void evalEvents (double timeIn, double[] oAmounts)
	{
	 ...
	 == 1.0) {
		...
		if (supportFunctions._gt(_y[2],(double)9/
	(double)4000)
	 == 1.0) {
	...
	private void InitializeDelays() { 
		...
		_eventDelay[1] = new TEventDelayDelegate(delegate { return (double)0.75; } );
		_eventType[1] = true;
