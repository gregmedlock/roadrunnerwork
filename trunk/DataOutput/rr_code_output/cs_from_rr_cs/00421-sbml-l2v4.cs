...
;
	...
	public void evalEvents (double timeIn, double[] oAmounts)
	{
		...
		if (supportFunctions._lt(_y[0],(double)0.65)
	 == 1.0) {
	...
	private void InitializeDelays() { 
		_eventDelay[0] = new TEventDelayDelegate(delegate { return (double)7.5; } );
		_eventType[0] = true;
		_eventPersistentType[0] = true;
		_eventDelay[1] = new TEventDelayDelegate(delegate { return (double)4.5; } );
		_eventType[1] = true;
