...
class TModel : IModel
{
	...
	private double[] _y = new double[1];            // Vector containing the concentrations of all floating species 
	private double[] _init_y = new double[1];            // Vector containing the initial concentrations of all floating species 
	private double[] _amounts = new double[1];      // Vector containing the amounts of all floating species 
	private double[] _bc = new double[2];           // Vector containing all the boundary species concentration values   
	private double[] _c = new double[1];            // Vector containing all the compartment values   
	private double[] _dydt = new double[1];         // Vector containing rates of changes of all species   
	...
	string[] variableTable = new string[1];
	string[] boundaryTable = new string[2];
	...
	public void evalEvents (double timeIn, double[] oAmounts)
	{
		...
		if (supportFunctions._lt(_y[0],(double)0.75)
	 == 1.0) {
		...
		if (supportFunctions._gt(_bc[1],(double)1.4)
	 == 1.0) {
	...
	private void InitializeDelays() { 
		_eventDelay[0] = new TEventDelayDelegate(delegate { return (double)1.5; } );
		_eventType[0] = true;
		_eventPersistentType[0] = true;
		_eventDelay[1] = new TEventDelayDelegate(delegate { return (double)1.5; } );
		_eventType[1] = true;
